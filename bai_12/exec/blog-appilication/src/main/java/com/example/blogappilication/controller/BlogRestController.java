...
public class BlogRestController {
    ...
    @GetMapping("")
    public ResponseEntity<List<Blog>> showHome(@RequestParam(defaultValue = "1") Integer page){
        List<Blog> blogs = iBlogService.findAll(page);
    ...
    public ResponseEntity<Iterable<Blog>> searchBlog(@RequestParam String search) {
        List<Blog> blogs = (List<Blog>) iBlogService.findAllByTitleContaining(search);
        return new ResponseEntity<>(blogs, HttpStatus.OK);
    }
}